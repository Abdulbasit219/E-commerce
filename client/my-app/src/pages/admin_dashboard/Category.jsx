...
const Category = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(`https://api-to2k.onrender.com/api/v1/category/create-category`, { name }, {
        headers: {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.put(`https://api-to2k.onrender.com/api/v1/category/update-category/${selectedCategory._id}`, { name: updatedName }, {
        headers: {
          Authorization: `${parseData.token}` //Pass the token through headers
        }
      });
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const res = await axios.delete(`https://api-to2k.onrender.com/api/v1/category/delete-category/${id}`);
      if (res.status === 200) {
